...
<script>
    ...
    // 單 JOB 單 Barcode：新增時，已使用的 JOB 直接在下拉選單停用；編輯時保留目前 JOB 可選。
    function refreshBarcodeJobAvailability() {
        const select = document.getElementById('barcode_job');
        if (!select) return;
        const currentRowId = String(document.getElementById('barcode_id')?.value || '');
        const usedJobs = new Set();
        document.querySelectorAll('#total_barcodes .barcode-check').forEach(cb => {
            const rowId = String(cb.dataset?.id || cb.dataset?.barcodeRowid || '');
            const jobId = String(cb.dataset?.jobId || '');
            if (!jobId || jobId === '-1') return;
            if (currentRowId && rowId === currentRowId) return;
            usedJobs.add(jobId);
        });
        Array.from(select.options).forEach(opt => {
            opt.disabled = (opt.value !== '-1' && usedJobs.has(String(opt.value)));
        });
    }
    ...
    try { refreshBarcodeJobAvailability(); } catch (_) {}
    }
    ...
    // 初次載入：對齊顯示狀態
    $(function(){ toggleBarcodeSeq(); refreshBarcodeJobAvailability(); });
    ...
    function delete_barcode_item() {
        ...
        // 從 data-id / data-barcode-rowid 取得 SQLite rowid，刪除單筆 Barcode。
        const barcodeIds = Array.from(checked).map(cb => {
            const raw = (cb.dataset && (cb.dataset.id || cb.dataset.barcodeRowid))
                ? (cb.dataset.id || cb.dataset.barcodeRowid)
                : (cb.getAttribute('data-id') || cb.getAttribute('data-barcode-rowid'));
            if (raw && /^\d+$/.test(raw)) return Number(raw);
            const m = (cb.id || '').match(/^barcode_check_(\d+)$/);
        ...
        alertify.confirm(
            ...
            function onOk() {
                ...
                $.ajax({
                    ...
                    method: "POST",
                    data: { barcode_id: barcodeIds }, // jQuery 會自動用 barcode_id[]
                    dataType: 'json',
                    success: function(response) {
