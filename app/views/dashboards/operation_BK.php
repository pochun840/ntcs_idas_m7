...
function renderChart(chart_mode, chart_info) {
    ...
    // === chart 7：Angle vs Time（與 mode 2 一樣用「局部角度」，但依 step 上色）===
    if (String(chart_mode) === "7") {

        // X = time
        const timeX = x_data_val.map(v => Number(v) || 0);

        // Y = local angle（不要累積，不要 total）
        ...
        myChart.setOption({
            ...
            xAxis: { type: 'value', name: 'Time', splitLine: { show: false } },
    ...
    // === chart 2：Angle vs Time（Step angle；X 軸 = 數值 Time）===
    if (String(chart_mode) === "2") {
        ...
        // === ★ 核心：偵測「角度跳變」並用 null 斷線 ===
        const BREAK_THRESHOLD = 300; 
        // 若角度跳變超過 300°，視為 step 結束 → 用 null 斷線避免畫垂直線
        ...
        const series = stepOrder.map((s, idx) => {
            ...
            for (let i = 0; i < timeX.length; i++) {
                ...
                if (stepsArr[i] !== s) continue;

                const t = timeX[i];
                const a = angleY[i];

                if (lastAngle !== null) {
                    const diff = Math.abs(a - lastAngle);
            ...
            return {
                ...
                type: 'line',
                showSymbol: false,
                connectNulls: false, // ★ 必須為 false，ECharts 才會尊重 null 斷線
                lineStyle: {
                    width: 2,
    ...
    // === chart 3：RPM vs Time（隱藏異常掉落，但最後 Step 不隱藏）===
    if (String(chart_mode) === "3") {

        const X   = x_data_val.map(v => Number(v) || 0);
        const RPM = y_data_val.map(v => Number(v) || 0);
        const stepsArr = (stepsRaw.length ? stepsRaw : X.map(() => 1)).map(s => {
            const m = /\d+/.exec(String(s));
            return m ? Number(m[0]) : 1;
        });

        const len = Math.min(X.length, RPM.length, stepsArr.length);
        ...
        for (let i = 0; i < len; i++) {
            ...
            // ★★★ 只對「不是最後一個 Step」做異常偵測
            if (s !== lastStep && i > 0) {

                const prevY = RPM[i - 1];

                // 異常垂直掉落 → 斷線
                if (Math.abs(prevY - y) > 200) {
                    buf.push([x, null]);  // 斷線 → ECharts 不畫線
                    continue;
                }
            }
        ...
        const series = segments.map(seg => ({
            ...
            lineStyle: {
                width: 2,
                color: palette[seg.step] || "#888"
            },
            data: seg.data
        }));

        myChart.setOption({
            tooltip: { trigger: 'item', axisPointer: { show: false } },
            axisPointer: { show: false },
            xAxis: { type: 'value', name: 'Time', splitLine: { show: false }},
            yAxis: { type: 'value', name: 'RPM', splitLine: { show: false }},
            series
        });
