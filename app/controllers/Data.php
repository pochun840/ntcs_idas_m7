...
class Data extends Controller
{
    ...
    /**
     * 取得目前登入者權限等級。
     * law = 3 為 Operator，只可瀏覽，不允許下載/匯出檔案。
     *
     * 注意：以 SESSION 的目前登入帳號為準。
     * 若 guest/admin 已登入，不可被舊的 cookie user_law=3 誤判成 Operator。
     */
    ...
    /**
     * 寫入前端可讀的 Operator 下載限制旗標。
     *
     * 說明：
     * - 後端仍以 denyDownloadIfOperator() 做最終防線。
     * - 這個 cookie 只用來讓 JS 立即把「匯出 / 下載」按鈕改成權限不足提示。
     * - 不放敏感資料，只放 1 / 空值。
     */
    ...
    /**
     * 後端下載防呆：避免使用者直接打 API URL 繞過前端按鈕限制。
     */
