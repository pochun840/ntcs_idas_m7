...
class Tools extends Controller
{
    ...
    /**
     * Run a callback only once using a filesystem flag + lock.
     *
     * @param string   $flagDir   Directory to store flag/lock files
     * @param string   $flagName  Flag filename (e.g. .tool_spec_synced)
     * @param callable $callback Function to execute once
     */
    public function runOnceWithFlag(string $flagDir, string $flagName, callable $callback): void
    {
        ...
        try {
            // 3) Exclusive lock to avoid concurrent execution
            if (!flock($lockFp, LOCK_EX)) {
                return;
            }

            // 4) Run only if flag does not exist
            if (!file_exists($flagFile)) {
                $callback();

                // 5) Write flag with timestamp
                $ok = @file_put_contents($flagFile, date('c') . PHP_EOL, LOCK_EX);
                if ($ok === false) {
                    error_log('[runOnceWithFlag] flag write failed: ' . $flagFile);
                }
            }
