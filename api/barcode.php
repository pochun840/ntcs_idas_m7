...
// Python 直接 POST JSON；空條碼不再從 barcode.txt 補讀。
$barcodeValue = $body['barcode'] ?? $body['Barcode'] ?? null;
if (!is_string($barcodeValue) || trim($barcodeValue) === '') {
    huarong_barcode_response(400, ['success' => false, 'message' => 'barcode 必須為非空字串']);
}
